...
\splitpagenoline{
    ...
    \centering
    $\blacktriangleright$
    Choose model $\expect{\y \vert \x} = m(\theta^\trans \x)$ and prior $\p(\theta)$\\
...
\splitpagenoline{
    ...
    \centering
    $\blacktriangleright$ Dependence on \surcol{$\y_i$} very complicated\\
...
\muhat_{\mrp} ={}& \meantar \yhat_j =
\meantar \x_j^\trans \betahat =
\sumsur
\underbrace{
\left(\meantar \x_j^\trans \left(\sum_{i'=1}^{\Nsur} \x_{i'} \x_{i'}^\trans \right)^{-1} \x_i\right)
}_{\w^\mrp_i}  \y_i \\
\end{aligned}
$$

Most existing literature on comparing CW and MrP focus on such linear models.
\footcite{gelman:2007:struggles,benmichael:2021:multilevel,chattopadhyay:2023:implied}
