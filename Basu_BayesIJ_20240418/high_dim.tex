...
both the slope $\blue{\infl_n}$ and the error $\red{\err(\w_n)}$ are $O_p(N^{-1})$,
\textbf{even if $\p(\gamma \vert \xvec)$ concentrates marginally,}
when $p(\lambda \vert \xvec, \gamma)$ does not concentrate. 
}
%
\end{frame}
