...
\theorem{
\textbf{Theorem 2 of \citet{giordano:2023:bayesij}  (paraphrase): }\\
If the posterior $\p(\theta \vert \xvec)$ satisfies a kind of Bayesian central limit theorem 
(BCLT),\footnote{Existing results are
sufficient for a \textit{particular weight} \citep{kass:1990:posteriorexpansions}. 
\citet{giordano:2023:bayesij} proves a kind of average convergence over all weights.}
then the map $\w_n \mapsto N \left( \expect{\p(\theta \vert \xvec, \w_n)}{f(\theta)} -
