...
data resampling).
% In my work, I build on classical statistical results which
% were originally used primarily for theory, adapting them for a modern problems,
% particularly taking advantage of the recent computational developments such as
% automatic differentiation.
% ideas that were mostly used for
...
% Unlike the posterior variance, the
...
% % \citep{waddell:2002:bayesphyloboot}.
% However, the principal existing approach
...
% \citep{wang:2005:inadequacy, turner:2011:two}
...
\subsection{Simplified and improved black-box variational inference}

Black box variational inference (BBVI) allows the use of variational inference
from little more than the implementation of a differentiable log joint
probability distribution.  Since BBVI objective functions involve the evaluation
of an intractable integral, standard BBVI implementations such as the
\texttt{vb} method implemented in the R software \texttt{Stan} rely on
stochastic gradient optimization using on draws from the variational
approximation. Unfortunately, the use of stochastic gradient optimization brings
about some difficulties: stochastic gradient can be more difficult to implement
and tune than standard deterministic optimization methods, convergence can be
difficult to assess, and, since the BBVI objective function cannot be directly
evaluated, one cannot directly compute the linear response covariances and prior
sensitivity measures from my research described above.
...
and optimize an approximate but deterministic objective.  With my coauthors in
\citet{giordano:2021:bbvi}, we show on a wide range of real-life datasets and
models that the deterministic objective can be optimized using off-the-shelf
methods more easily and, often, more quickly, than stochastic gradient descent
on the exact objective. Further, the use of a deterministic objective allows us
to compute linear response covariances, rectifying the badly mis-estimated
posterior uncertainty estimates produced by stochastic BBVI. Finally, we show
that the error induced by using an approximate but deterministic objective is
both small and quantifiable, since standard frequentist measures of uncertainty
can tell us how much error was induced by using an empirical expectation in
place of an exact one.
