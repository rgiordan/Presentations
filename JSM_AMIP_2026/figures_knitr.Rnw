...
mcmix_cache <- TRUE && (!rerun_all)
coreset_cache <- FALSE && (!rerun_all)
...
<<sim-refit-one, fig.align='center', cache=sim_cache, fig.show='hold'>>=
print(plot1)
@
}

\newcommand{\SimRefitTwo}{
<<sim-refit-two, fig.align='center', cache=sim_cache, fig.show='hold'>>=
print(plot2)
SetFullImageSize()
@
}
...
<<mx-hist-cap>>=
SetImageSize(aspect_ratio=0.8 * base_aspect_ratio)
@
<<mx-hist, cache=coreset_cache, fig.show='hold'>>=
source("figures_knitr/coresets/mx_histogram.R",
       echo=knitr_debug, print.eval=TRUE)
SetFullImageSize()
@
}
...
SetImageSize(aspect_ratio=0.8 * base_aspect_ratio)
@
<<greedy-drop, cache=coreset_cache, fig.show='hold', fig.cap='MX profit analysis'>>=
print(greedy_coreset_plot)
SetFullImageSize()
@
}
...
SetImageSize(aspect_ratio=0.8 * base_aspect_ratio)
@
<<influential-keep, cache=coreset_cache, fig.show='hold', fig.cap='MX profit analysis'>>=
print(influential_coreset_plot)
SetFullImageSize()
@
}
...
SetImageSize(aspect_ratio=0.8 * base_aspect_ratio)
@
<<gd-path, cache=coreset_cache, fig.show='hold', fig.cap='MX profit analysis'>>=
print(gd_plot)
SetFullImageSize()
@
}
...
SetImageSize(aspect_ratio=0.8 * base_aspect_ratio)
@
<<gd-refit, cache=coreset_cache, fig.show='hold', fig.cap='MX profit analysis'>>=
print(gd_refit_plot)
SetFullImageSize()
@
}
