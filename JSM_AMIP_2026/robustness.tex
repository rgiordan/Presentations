...
Formalization of distributional robustness:
...
% \begin{align*}
% %
% T'(F_0) \shift = \int \infl(\x; F_0) \shift(\dee d)
% %
% \end{align*}
% %
...
But suppose we are interested in Bolivia, with population $\flim + \shift$!
\pause
%
\begin{align*}
\textrm{Error for Bolivia} ={}& T(\fhat) - T(\flim + \shift)\\
={}&
\underbrace{T(\fhat) - T(\flim)}_{\textrm{Sampling variability }\rightarrow 0} + 
\underbrace{T(\flim) - T(\flim + \shift)}_{\textrm{Distribution shift } \ne 0} 
\end{align*}

\textbf{How big can $\shift$ be?}\\[1em]

\only<3->{
Classical robustness mostly considers
distribution space neighborhoods (contamination neighborhoods,
total variation distance, KL divergence, Kolmogorov distance,
...
(What is the Levy-Prohorov distance between MX and Bolivia?)\\[1em]

\onslide<5->{
    Note that you can't get Bolivia by dropping points from Mexico, either.
}
}
...
\textbf{How big can $\shift$ be?}
Data dropping gives a simple notion of ``size:''\\[1em]

\textbf{Define} $\vnorm{\shift}$ = Proportion of points you need to drop to
produce a local change in $T$ as large as that produced by $\shift$. 
...
% $$
% \alpha^*(\shift) = \inf\{\alpha: \textrm{AMIP}(\alpha) \ge \shift\},
% $$
% with $\alpha^*(\shift) = 0$ for $\shift = 0$ and $\infty$ if
% the set is empty.
% \end{minipage}
{
\begin{center}
...
% Noting that $T(\flim + \shift) - T(\flim) \approx \int \shift(\d) \infl(\d) \flim(\dee \d)$,
% define
% $$
% \vnorm{\shift} := \alpha^*\left(  \int \shift(\d) \infl(\d) \flim(\dee \d)\right).
% $$

\pause

\begin{itemize}
    \item Easy to think about
    \item Easy to compute (but only locally meaningful)
    \item Non-negative, sub-additive, multiplicatively non-decreasing
    \item Not a norm ($\shift \ne 0$ can have $\vnorm{\shift} = 0$) nor a
    semi-norm (not symmetric, not homogeneous)
\end{itemize}
