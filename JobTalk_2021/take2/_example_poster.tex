...
\setlength{\twocolwid}{0.64\paperwidth} % Width of two columns
\setlength{\threecolwid}{0.96\paperwidth} % Width of three columns
...
\title{An Automatic Finite-Sample Robustness Metric: \\Can Dropping a Little Data
Change Conclusions?} % Poster title

\author{Tamara Broderick$^{\dagger}$,
        Ryan Giordano$^{* \dagger}$,
        Rachael Meager$^{* \ddagger}$} % Author(s)

\institute{$^{*}$ Equal contribution first authors \hspace{2em}
           $^{\dagger}$ MIT Department of EECS \hspace{2em}
           $^{\ddagger}$ LSE Department of Economics} % Institution(s)
...
% split into two columns twice - the [t] option aligns each column's
% content to the top
\begin{columns}[t]
...
amet placerat quis, laoreet id est.
