...
<<>>=
figcap <- paste0("Weight comparison for the Name Change dataset")
@
...
<<>>=
figcap <- paste0("Weight comparison for the Gay Marriage dataset")
@
...
<<>>=
figcap <- paste0("Region partial pooling for the Name Change dataset")
@
<<poolingplot, cache=cache, fig.show='hold', fig.cap=figcap>>=
source("figures_knitr/partial_pooling.R")
@
}
